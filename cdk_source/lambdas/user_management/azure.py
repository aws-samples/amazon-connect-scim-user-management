...
def lambda_handler(event, context):
    ...
    if method == 'GET':
        ...
        if (event['pathParameters'] and event['pathParameters']['proxy'] and event['pathParameters']['proxy'] !='Users'):      # noqa: E501
    ...
    if method == 'PATCH':
        ...
        for info in user_update_info['Operations']:
            user_status = info['value']
            user_action = info['op']
            user_path= info['path']

        LOGGER.info("User status is ..... %s",user_status)
        if (user_status == 'No' and user_action == 'Replace' and user_path == 'active'):
